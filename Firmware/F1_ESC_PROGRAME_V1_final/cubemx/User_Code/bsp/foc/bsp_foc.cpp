...
foc motor_1612(&pwm_u, &pwm_v, &pwm_w, 7, REVERSE, M_EN_GPIO_Port, M_EN_Pin);
pid pid_out_1612(2.0f, 0.0f, 30.0f, 0.0f, 300.0f);
// pid pid_in_1612(0.003, 0.0001, 0, 0.5, 0.5);
pid pid_in_1612(0.003f, 0.00005f, 0.0f, 0.5f, 0.5f);
pid pid_IQ(-2.0f, -2.0f, 0.0f, 3.5f, 3.5f);
pid pid_ID(-2.0f, -2.0f, 0.0f, 3.5f, 3.5f);
...
float foc::init_ZeroElectricalAngle(uint16_t delaytime)
{
	...
	Clark_Park_Inverse(0, voltage_limit / 2, 0);
	SvpwmCtrl();
	run_UVW_T();
...
void foc::Clark_Park_Inverse(float U_q_input, float U_d_input, float angle_el)
{
	...
	this->U_d = _constrain(U_d_input, -(voltage_power_supply) / 2, (voltage_power_supply) / 2);
...
void foc::SvpwmCtrl(void)
{
	...
	this->Svpwm_Mod.udc = this->voltage_limit;	// 输出的母线电压
...
void foc::run(void)
{
	...
	// Clark_Park_Inverse(0, 4, 0); // 克拉克帕克变换
